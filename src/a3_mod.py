...
def forward_pass(W1, W2, W3, b1, b2, b3, x):
    ...
    # #######
    # use the LogSumExp Trick by choosing the a value as the max of the input into the log-softmax, which is the Fhat 
    a_value = np.max(Fhat, axis=1, keepdims=True)
    log_softmax_value = Fhat-(a_value+np.log(np.sum(np.exp(Fhat-a_value), axis=1, keepdims=True)))
...
def negative_log_likelihood(W1, W2, W3, b1, b2, b3, x, y):
    ...
    nll = -np.sum(np.sum(np.multiply(y, Fhat), axis=1), axis=0)
    return nll
